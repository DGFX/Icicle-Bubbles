...
void main() {
    ...
    vec3 position = mix(vec3(0.0, DEFAULT_Y, 0.0), positionInfo.xyz, smoothstep(0.0, 0.3, 1.0));
    float life = positionInfo.a - dieSpeed;
    vec3 followPosition = mix(vec3(0.0, DEFAULT_Y, 0.0), mouse3d, smoothstep(0.2, 0.7, 1.0));
    ...
    if(life < 0.0) {
        ...
        position += delta * (0.005 + life * 0.01) * attraction * (1.0 - smoothstep(100.0, 500.0, deltaLength)); // Interesting effect
